...
public class BankForexLimitUsageServiceImpl implements BankForexLimitUsageService {
	...
	private Map<String, Object> setModelToMap(BankForexLimitModel model) {
		...
		BigDecimal limitUsageEq = (BigDecimal)ValueUtils.getValue(model.getLimitUsageEquivalent(), BigDecimal.ZERO);		
		BigDecimal maxBuyLimit = (BigDecimal)ValueUtils.getValue(model.getMaxBuyLimit(), BigDecimal.ZERO);
		BigDecimal maxSellLimit = (BigDecimal)ValueUtils.getValue(model.getMaxSellLimit(), BigDecimal.ZERO);
		BigDecimal remainingBuyLimit = maxBuyLimit.subtract(limitUsageEq);
		BigDecimal remainingSellLimit = maxSellLimit.add(limitUsageEq);
		
		map.put("limitUsage", ValueUtils.getValue(model.getLimitUsage(), BigDecimal.ZERO));
		map.put("limitUsageEquivalent", limitUsageEq);
		map.put("maxBuyLimit", maxBuyLimit);
		map.put("maxSellLimit",maxSellLimit);
		map.put("remainingBuyLimit", remainingBuyLimit);
		map.put("remainingSellLimit",remainingSellLimit);
	...
	public Map<String, Object> submit(Map<String, Object> map) throws ApplicationException, BusinessException {
		...
		try {
			...
			if (map.get(ApplicationConstants.WF_ACTION).equals(ApplicationConstants.WF_ACTION_UPDATE)) {
				vo.setAction((String)map.get("wfAction"));
				...
				vo.setAction(ApplicationConstants.WF_ACTION_RESET);
	...
	private PendingTaskVO setPendingTaskVO(Map<String, Object> map) throws Exception {
		...
		vo.setService("BankForexLimitUsageSC");
		...
		try {
			...
			if (ApplicationConstants.WF_ACTION_UPDATE.equals(vo.getAction())) {
				...
				for(Map<String, Object> bankForexLimitMap : limitList){
					...
					limitModel.setlimitUsageEquivalent(limitUsageEqOld.subtract(coverUsageEq));
			...
			} else if (ApplicationConstants.WF_ACTION_RESET.equals(vo.getAction())) {
				
				for(Map<String, Object> bankForexLimitMap : limitList){
					BankForexLimitModel limitModel = getExistingRecord((String)bankForexLimitMap.get("id"), true);
					limitModel.setLimitUsage(BigDecimal.ZERO);
					limitModel.setlimitUsageEquivalent(BigDecimal.ZERO);
					limitModel.setUpdatedDate(DateUtils.getCurrentTimestamp());
					limitModel.setUpdatedBy(vo.getCreatedBy());
