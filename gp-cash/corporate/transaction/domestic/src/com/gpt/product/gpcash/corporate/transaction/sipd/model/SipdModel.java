package com.gpt.product.gpcash.corporate.transaction.sipd.model;
...
@Entity
@Table(name = "TRX_SIPD")
public class SipdModel implements Serializable {
	...
	@Column(name = "USER_ID_SIPD")
	protected String userIdSipd;
	
	@ElementCollection(fetch = FetchType.LAZY)
	@CollectionTable(
		name = "TRX_SIPD_SENDERINFO", 
		joinColumns = @JoinColumn(name="PARENT_ID")
    )
	@OrderColumn(name = "IDX")
	@Column(name = "VALUES_", length = 1024)
	protected List<String> senderInfo;
	
	@ElementCollection(fetch = FetchType.LAZY)
	@CollectionTable(
		name = "TRX_SIPD_RECIPIENTINFO", 
		joinColumns = @JoinColumn(name="PARENT_ID")
    )
	@OrderColumn(name = "IDX")
	@Column(name = "VALUES_", length = 1024)
	protected List<String> recipientInfo;
	
	@ElementCollection(fetch = FetchType.LAZY)
	@CollectionTable(
		name = "TRX_SIPD_ADDITIONALDATA", 
		joinColumns = @JoinColumn(name="PARENT_ID")
    )
	@OrderColumn(name = "IDX")
	@Column(name = "VALUES_", length = 1024)
	...
	public String getSipdId() {
		return sipdId;
	}

	public void setSipdId(String sipdId) {
		this.sipdId = sipdId;
	}
	...
	public String getTxPartnerId() {
		return txPartnerId;
	}

	public void setTxPartnerId(String txPartnerId) {
		this.txPartnerId = txPartnerId;
	}
	...
	public String getUserIdSipd() {
		return userIdSipd;
	}

	public void setUserIdSipd(String userIdSipd) {
		this.userIdSipd = userIdSipd;
	}

	public List<String> getSenderInfo() {
		return senderInfo;
	}

	public void setSenderInfo(List<String> senderInfo) {
		this.senderInfo = senderInfo;
	}

	public String getSenderInfoStr() {
		if(senderInfo != null && !senderInfo.isEmpty()) {
			return Helper.glueStringChopsBackTogether(senderInfo);
		}
		return null;
	}
	
	public void setSenderInfoStr(String senderInfo) throws Exception {
		List<String> listOfRecord = Helper.chopItUp(senderInfo);

		if (ValueUtils.hasValue(listOfRecord)) {
			setSenderInfo(listOfRecord);
		}
	}
	...
	public String getRecipientInfoStr() {
		if(recipientInfo != null && !recipientInfo.isEmpty()) {
			return Helper.glueStringChopsBackTogether(recipientInfo);
		}
		return null;
	}
	
	public void setRecipientInfoStr(String recipientInfo) throws Exception {
		List<String> listOfRecord = Helper.chopItUp(recipientInfo);

		if (ValueUtils.hasValue(listOfRecord)) {
			setRecipientInfo(listOfRecord);
		}
	}
	...
	public String getAdditionalDataStr() {
		if(additionalData != null && !additionalData.isEmpty()) {
			return Helper.glueStringChopsBackTogether(additionalData);
		}
		return null;
	}
	
	public void setAdditionalDataStr(String additionalData) throws Exception {
		List<String> listOfRecord = Helper.chopItUp(additionalData);

		if (ValueUtils.hasValue(listOfRecord)) {
			setAdditionalData(listOfRecord);
		}
	}
