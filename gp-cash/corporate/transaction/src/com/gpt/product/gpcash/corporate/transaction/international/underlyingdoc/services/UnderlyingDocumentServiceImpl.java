...
import com.gpt.product.gpcash.corporate.transaction.international.underlyingdoc.repository.UnderlyingDocumentRepository;

@Service
@Transactional(rollbackFor = Exception.class)
public class UnderlyingDocumentServiceImpl implements UnderlyingDocumentService {
	@Autowired
	private UnderlyingDocumentRepository underlyingRepo;
		...
		try {
			...
			if (map.get(ApplicationConstants.WF_ACTION).equals(ApplicationConstants.WF_ACTION_CREATE)) {
				...
				for (Map<String, Object> documentMap : documentList) {
					...
					UnderlyingDocumentModel underlying = setMapToModel(documentMap, corporateId);
				...
				for (Map<String, Object> documentMap : documentList) {
					...
					UnderlyingDocumentModel underlying = underlyingRepo.findOne(id);
...
//					underlyingRepo.delete(id);
	...
	private List<Map<String, Object>> setModelToMap(List<UnderlyingDocumentModel> list) {
		List<Map<String, Object>> resultList = new ArrayList<>();

		for (UnderlyingDocumentModel model : list) {
	...
	private Map<String, Object> setModelToMap(UnderlyingDocumentModel model ) {
		...
		map.put(ApplicationConstants.CORP_ID, model.getCorporate().getId());
		map.put("corporateName", model.getCorporate().getName());
	...
	private UnderlyingDocumentModel setMapToModel(Map<String, Object> map, String corporateId) throws Exception {
		UnderlyingDocumentModel underlying = new UnderlyingDocumentModel();

		CorporateModel corporate = new CorporateModel();
		corporate.setId(corporateId);
		underlying.setCorporate(corporate);
	...
	public Map<String, Object> search(Map<String, Object> map) throws ApplicationException, BusinessException {
		Map<String, Object> resultMap = new HashMap<>();
		try {
			Page<UnderlyingDocumentModel> result = underlyingRepo.findByCorporateIdAndDeleteFlagOrderByUnderlyingAmountAsc((String) map.get(ApplicationConstants.CORP_ID), ApplicationConstants.NO, PagingUtils.createPageRequest(map));
