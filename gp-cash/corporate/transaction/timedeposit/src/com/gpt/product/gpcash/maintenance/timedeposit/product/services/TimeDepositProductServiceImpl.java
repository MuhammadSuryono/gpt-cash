...
import com.gpt.product.gpcash.maintenance.timedeposit.product.repository.TimeDepositProductRepository;
import com.gpt.product.gpcash.maintenance.timedeposit.product.repository.TimeDepositTermRepository;

@Service
@Transactional(rollbackFor = Exception.class)
public class TimeDepositProductServiceImpl implements TimeDepositProductService {
	private final Logger logger = LoggerFactory.getLogger(getClass());

	@Autowired
	private TimeDepositProductRepository productRepo;

	@Autowired
	private TimeDepositTermRepository termRepo;
	...
	public Map<String, Object> search(Map<String, Object> map) throws ApplicationException, BusinessException {
		...
		try {
			Page<TimeDepositProductModel> result = productRepo.search(map, PagingUtils.createPageRequest(map));
	...
	private List<Map<String, Object>> setModelToMap(List<TimeDepositProductModel> list, boolean isGetDetail) {
		List<Map<String, Object>> resultList = new ArrayList<>();

		for (TimeDepositProductModel model : list) {
	...
	private Map<String, Object> setModelToMap(TimeDepositProductModel model, boolean isGetDetail) {
		...
		if(isGetDetail){
			...
			try {
				List<TimeDepositTermModel> termList = termRepo.searchTermByProductCode(model.getCode());
	...
	public Map<String, Object> submit(Map<String, Object> map) throws ApplicationException, BusinessException {
		...
		try {
			...
			if (map.get(ApplicationConstants.WF_ACTION).equals(ApplicationConstants.WF_ACTION_CREATE)) {
				...
				TimeDepositProductModel productOld = getExistingRecord((String) map.get(ApplicationConstants.STR_CODE), true);
				...
				for (Map<String, Object> termMap : termList) {
					TimeDepositTermModel termOld = getExistingTermRecord((String) termMap.get(ApplicationConstants.STR_CODE), productOld.getCode(), false);
					if (termOld !=null) {
	...
	private TimeDepositProductModel getExistingRecord(String code, boolean isThrowError) throws BusinessException {
		TimeDepositProductModel model = productRepo.findOne(code);
	...
	private TimeDepositTermModel getExistingTermRecord(String code, String productCode, boolean isThrowError) throws Exception {
		TimeDepositTermModel model = termRepo.findByCodeAndProductType_code(code, productCode);
	...
	private PendingTaskVO setPendingTaskVO(Map<String, Object> map) {
		...
		vo.setService("TimeDepositProductSC");

		return vo;
	}

	private TimeDepositProductModel setMapToModel(Map<String, Object> map) {
	...
	private void checkUniqueRecord(String productCode) throws Exception {
		TimeDepositProductModel product = productRepo.findOne(productCode);
	...
	public PendingTaskVO approve(PendingTaskVO vo) throws ApplicationException, BusinessException {
		try {
			...
			if (ApplicationConstants.WF_ACTION_CREATE.equals(vo.getAction())) {
				TimeDepositProductModel product = setMapToModel(map);
				TimeDepositProductModel productExisting = productRepo.findOne(product.getCode());
				...
				List<Map<String, Object>> termList = (ArrayList<Map<String, Object>>) map.get("termList");
				ArrayList<TimeDepositTermModel> terms = new ArrayList<>();
				TimeDepositProductModel productExisting = getExistingRecord((String) map.get(ApplicationConstants.STR_CODE), true);
				...
				if (termList !=null) {
					...
					for (Map<String, Object> termMap : termList) {
						TimeDepositTermModel termNew = setMapToModelTerm(termMap);
					...
					TimeDepositProductModel productNew = setMapToModel(map);
				...
				TimeDepositProductModel product = getExistingRecord((String) map.get(ApplicationConstants.STR_CODE), true);
	...
	private TimeDepositTermModel setMapToModelTerm(Map<String, Object> termMap) {
		
		TimeDepositTermModel term = new TimeDepositTermModel();
	...
	public Map<String, Object> searchTermByProduct(Map<String, Object> map) throws ApplicationException, BusinessException {
		Map<String, Object> resultMap = new HashMap<>();
		try {
			List<TimeDepositTermModel> result = termRepo.searchTermByProductCode((String) map.get("productCode"));
...
//			PagingUtils.setPagingInfo(resultMap, result);
	...
	private List<Map<String, Object>> setModelToMapTerm(List<TimeDepositTermModel> list, boolean isGetDetail) throws Exception {
		List<Map<String, Object>> resultList = new ArrayList<>();

		for (TimeDepositTermModel model : list) {
	...
	private Map<String, Object> setModelToMapTerm(TimeDepositTermModel model, boolean isGetDetail) throws Exception {
		Map<String, Object> map = new HashMap<>();
