...
public class BalanceSummaryServiceImpl implements BalanceSummaryService {
	...
	public Map<String, Object> singleAccount(Map<String, Object> map) throws ApplicationException, BusinessException {
		try {			
			...
			AccountModel accountModel = corporateAccountGroupService.searchAccountByAccountNoForInquiryOnly(corpId,
					userCode, accountNo, true);
	...
	public Map<String, Object> multiAccountByBranch(Map<String, Object> map) throws ApplicationException, BusinessException {
		try {
			...
			List<CorporateAccountGroupDetailModel> accountModels = null;

			if (ValueUtils.hasValue(accountBranchCode)) {
				accountModels = corporateAccountGroupService.searchCorporateAccountGroupDetailByBranchForInquiryOnly(corpId, userCode, accountBranchCode);
			} else {
				accountModels = corporateAccountGroupService.searchCorporateAccountGroupDetailForInquiryOnly(corpId,userCode);
			}

			if (ValueUtils.hasValue(accountModels)) {
				List<Map<String, Object>> accountList = new ArrayList<>();

				for (CorporateAccountGroupDetailModel corporateAccountGroupDetail : accountModels) {
					Map<String, Object> account = new HashMap<>(9, 1);

					AccountModel accountModel = corporateAccountGroupDetail.getCorporateAccount().getAccount();
					account.put("accountNo", accountModel.getAccountNo());
					account.put("accountName", accountModel.getAccountName());
	...
	public Map<String, Object> multiAccountByAccountType(Map<String, Object> map) throws ApplicationException, BusinessException {
		try {
			...
			if (ValueUtils.hasValue(accountTypeCode)) {
				...
				accountModels = corporateAccountGroupService.searchCorporateAccountGroupDetailByAccountTypeForInquiryOnly(corpId, userCode,	accountTypeList);
			} else {
				accountModels = corporateAccountGroupService.searchCorporateAccountGroupDetailForInquiryOnly(corpId, userCode);
			}
	...
	private Map<String, Object> constructSummaryResult(List<Map<String, Object>> accountList) {
		...
		for (Map<String, Object> account : accountList) {
			...
			if (accountType.equals(ApplicationConstants.ACCOUNT_TYPE_CURRENT)
				...
				if ("1".equals(maturityInstruction)) // Interest add to Principle		
					maturityInstruction = "GPT-0100205"; 
				else if ("2".equals(maturityInstruction)) // Interest add to Source Account
