package com.gpt.product.gpcash.retail.logging.activity.controller;
...
import com.gpt.product.gpcash.retail.logging.CustomerActivityLogActivityType;

@RestController
public class CustomerActivityLogController extends CustomerUserBaseController {
	
	protected static final String menuCode = "MNU_R_GPCASH_F_NON_FIN";
	...
	public DeferredResult<Map<String, Object>> genericHandler(HttpServletRequest request, @PathVariable String method, @RequestBody Map<String, Object> param) {
		return invoke("CustomerActivityLogSC", method, param);
	}
	
	@RequestMapping(baseCustUserUrl + "/" + menuCode + "/getMenuType")
	...
	@RequestMapping(baseCustUserUrl + "/" + menuCode + "/getActivityType")
	public List<Map<String, String>> getActivityType(Locale locale) {
		List<Map<String, String>> result = activityType.get(locale);
		if (result == null) {
			CustomerActivityLogActivityType[] values = CustomerActivityLogActivityType.values();
			result = new ArrayList<>(values.length);

			for (CustomerActivityLogActivityType value : values) {
				Map<String, String> vals = new HashMap<>(2, 1);
	...
	@RequestMapping(baseCustUserUrl + "/" + menuCode + "/getStatus")
