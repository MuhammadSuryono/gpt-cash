...
import com.gpt.product.gpcash.retail.transaction.international.services.CustomerInternationalTransferService;

@Validate
@Service
public class CustomerInternationalTransferStatusSCImpl implements CustomerInternationalTransferStatusSC {

	@Autowired
	private CustomerInternationalTransferService internationalService;
	...
	@Input({ 
		...
		@Variable(name = ApplicationConstants.LOGIN_USERID, format = Format.UPPER_CASE),
		@Variable(name = ApplicationConstants.WF_ACTION, options = { 
			CustomerInternationalConstants.WF_ACTION_PROCESS
		}),
		@Variable(name = ApplicationConstants.STR_MENUCODE, options = menuCode) })
	@Output({
	...
	@Input({ 
		...
		@Variable(name = ApplicationConstants.LOGIN_USERID, format = Format.UPPER_CASE),
		@Variable(name = ApplicationConstants.WF_ACTION, options = { 
				CustomerInternationalConstants.WF_ACTION_DECLINE
		}),
		@Variable(name = ApplicationConstants.STR_MENUCODE, options = menuCode) })
	@Output({
		@Variable(name = ApplicationConstants.WF_FIELD_REFERENCE_NO),
